...
Um sistema de **RAG (Retrieval-Augmented Generation)** que processa documentos PDF e responde perguntas em linguagem natural. O pipeline inclui conversão para Markdown (MarkItDown), segmentação de texto, embeddings com **nomic-embed-text-v1.5** (sentence-transformers), indexação vetorial com **FAISS** e geração de respostas via **Hermes 3 3B** (LM Studio). Tudo rodando **100% offline**, com total privacidade e controle dos seus dados.
...
|------------|------------|
| Extração de texto | MarkItDown (Microsoft) |
| Segmentação | RecursiveCharacterTextSplitter (custom) |
| Embeddings | Nomic Embed (sentence-transformers) |
| Indexação | FAISS |
| Geração | Hermes 3 (LM Studio) |
| Interface | Streamlit |
...
1. **Clone o repositório**
   ...
   ```bash
   git clone https://github.com/Gussnogue/assistente-administrativo-rag-ialocal-faiss.git
   cd assistente-administrativo-rag-ialocal-faiss
